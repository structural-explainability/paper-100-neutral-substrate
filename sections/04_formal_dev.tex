...
and substrate-layer commitments.
...
then there may exist admissible frameworks that derive
...
proposition as a substrate-layer commitment,
...
it is incompatible with any framework that treats $e_1$ and $e_2$ as merely correlated
...
the legal framework's background axiom (in this instance) that
mere temporal precedence without established mechanism
does not constitute causation.
These commitments are inconsistent under that framework:
...
This is a substrate-layer fact about the existence of an assertion,
...
of the same occurrence without contradiction at the substrate layer,
...
By Lemma~\ref{lem:contestability}, $p$ is framework-variant,
and therefore there exist admissible frameworks
$\mathcal{F}_1, \mathcal{F}_2 \in \mathbb{F}$
such that
$\mathcal{F}_1 \vdash p$ and
$\mathcal{F}_2 \vdash \neg p$.
Since
$\mathcal{S} \vdash p$ and $\mathcal{F}_2 \vdash \neg p$,
...
at the substrate layer and instead represents only
framework-invariant referential structure:
...
Let $\mathcal{F} \in \mathbb{F}$ be an admissible framework.
Since $\mathcal{S}$ asserts only framework-invariant propositions,
and $\mathcal{F}$ is internally consistent,
no proposition asserted by $\mathcal{S}$ is contradicted by $\mathcal{F}$.
Therefore, $\mathcal{S} \cup \mathcal{F} \not\vdash \bot$.
...
clarifying the exclusions and requirements at the substrate layer,
